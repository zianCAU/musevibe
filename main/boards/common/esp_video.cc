...
std::string EspVideo::GetJpeg() {
    // CALLER CONVENTION: Must be called after Capture() returns (not concurrently).
    // frame_ is written by Capture() and read here; no mutex protects frame_ because
    // the MCP framework executes tool callbacks sequentially on the main task.
    if (!frame_.data || frame_.len == 0) {
        return "";
    }

    // Use a simple std::string accumulator as output buffer
    ...
    if (!ok || jpeg_data.empty()) {
        ESP_LOGE(TAG, "GetJpeg: JPEG encode failed");
        return "";
    }
    ESP_LOGI(TAG, "GetJpeg: encoded %zu bytes", jpeg_data.size());
    return jpeg_data;
}
