...
// 演奏同步手势（来自 otto_controller.cc）
extern void OttoPlayMusicGesture(bool is_strum, int bpm);
// 情绪表达（来自 otto_controller.cc）
extern void OttoExpressEmotion(const char* emotion);
// 陪伴模式触摸反馈（来自 otto_controller.cc）
extern void OttoCompanionReaction(int pad_index);
// 对话手势后台任务（来自 otto_controller.cc）
extern void OttoStartTalkGestures();
extern void OttoStopTalkGestures();

// Forward declaration for use in constructor
static class OttoRobot* g_otto_robot = nullptr;

class OttoRobot : public WifiBoard {
private:
    enum AppMode { kMusic, kCompanion };
    AppMode app_mode_ = kMusic;
    ...
    OttoCameraType camera_type_ = OTTO_CAMERA_NONE;
    TaskHandle_t camera_preview_task_ = nullptr;
    ...
    bool DetectHardwareVersion() {
        ...
        for (size_t i = 0; i < sizeof(camera_addresses); i++) {
            ...
            if (ret == ESP_OK) {
                ...
                // 尝试 OV2640 方式（8位寄存器地址 0x0A，读2字节PID）
                ...
                if (ret == ESP_OK && (data[0] != 0 || data[1] != 0)) {
                    ...
                    ESP_LOGI(TAG, "检测到摄像头 (OV2640方式) PID=0x%04X (地址=0x%02X)", detected_pid, addr);
                    camera_found = true;
                    i2c_master_bus_rm_device(dev_handle);
                    break;
                }

                // 尝试 OV3660 方式（16位寄存器地址 0x300A/0x300B，各读1字节PID）
                uint8_t reg_high[2] = {0x30, 0x0A};
                uint8_t reg_low[2]  = {0x30, 0x0B};
                uint8_t pid_high = 0, pid_low = 0;
                ret = i2c_master_transmit_receive(dev_handle, reg_high, 2, &pid_high, 1, 200);
                if (ret == ESP_OK) {
                    ret = i2c_master_transmit_receive(dev_handle, reg_low, 2, &pid_low, 1, 200);
                    if (ret == ESP_OK) {
                        detected_pid = (pid_high << 8) | pid_low;
                        if (detected_pid != 0) {
                            ESP_LOGI(TAG, "检测到摄像头 (OV3660方式) PID=0x%04X (地址=0x%02X)", detected_pid, addr);
                            camera_found = true;
                            i2c_master_bus_rm_device(dev_handle);
                            break;
                        }
    ...
    void InitializeLcdDisplay() {
        ...
        // ST7789 有 320 行 VRAM，只显示 240 行。
        // MIRROR_Y=true 时读取方向从底部开始，需要补偿 80 行偏移
        // 通过 set_gap 而非 OFFSET_Y（OFFSET_Y 会偏移 LVGL 坐标系，set_gap 只影响 VRAM 地址）
#if DISPLAY_MIRROR_Y
    ...
    void InitializeButtons() {
        ...
        boot_button_.OnDoubleClick([this]() {
            auto* muse = static_cast<MuseDisplay*>(display_);
            if (app_mode_ == kMusic) {
                app_mode_ = kCompanion;
                muse->ShowCompanionMode();
                ESP_LOGI(TAG, "切换到陪伴模式");
            } else {
                app_mode_ = kMusic;
                muse->ShowMain();
                ESP_LOGI(TAG, "切换到弹奏模式");
            }
        });
    }
    ...
    void InitializeOttoController() {
        ...
        // Hook: start/stop random talk gestures when speaking state changes
        Application::GetInstance().AddDeviceStateListener(
            [](DeviceState old_state, DeviceState new_state) {
                if (new_state == kDeviceStateSpeaking) {
                    OttoStartTalkGestures();
                } else if (old_state == kDeviceStateSpeaking) {
                    OttoStopTalkGestures();
                }
            });
    }
    ...
    void InitializeChordPlayer() {
        ...
        // 注册演奏同步手势回调
        chord_player_->SetGestureCallback([](bool is_strum, int bpm) {
            OttoPlayMusicGesture(is_strum, bpm);
        });
    }

    void InitializeTouchController() {
        // 4 companion mode messages, one per pad (C / Am / F / G)
        static const char* kCompanionMessages[4] = {
            "摸肚子",
            "戳腹肌",
            "蹭肚子",
            "揉肚子",
        ...
        touch_controller_ = new TouchChordController(
            [this](Chord chord) {
                if (app_mode_ == kCompanion) {
                    // 陪伴模式：触摸腹肌 → 体态反馈 + 发送AI消息
                    int pad = static_cast<int>(chord);
                    ESP_LOGI("OttoRobot", "陪伴触摸: pad %d", pad);
                    OttoCompanionReaction(pad);
                    static_cast<MuseDisplay*>(display_)->HighlightChord(chord);

                    auto& app = Application::GetInstance();
                    auto state = app.GetDeviceState();

                    if (state == kDeviceStateIdle) {
                        // 待机状态：第一次触摸先唤醒，开启对话通道
                        ESP_LOGI("OttoRobot", "陪伴模式：待机→唤醒");
                        app.ToggleChatState();
                    } else if (pad >= 0 && pad < 4) {
                        // 已在对话中：发送亲昵触摸消息
                        app.SendCompanionTouch(kCompanionMessages[pad]);
                    }
                } else {
                    // 弹奏模式：正常和弦
            ...
            [this](bool up) {
                static_cast<MuseDisplay*>(display_)->ScrollLyrics(up ? -40 : 40);
            }
        );
...
public:
    ...
    void StartCameraPreview() {
        if (!has_camera_ || camera_ == nullptr) return;
        if (camera_preview_task_ != nullptr) return;  // already running
        xTaskCreate(
            [](void* arg) {
                auto* self = static_cast<OttoRobot*>(arg);
                ESP_LOGI(TAG, "Camera preview task started");
                while (true) {
                    self->camera_->Capture();
                    vTaskDelay(pdMS_TO_TICKS(100));  // ~10 fps
                }
            },
            "cam_preview", 1024 * 6, this, 3, &camera_preview_task_);
        ESP_LOGI(TAG, "Camera preview started");
    }

    void StopCameraPreview() {
        if (camera_preview_task_ != nullptr) {
            vTaskDelete(camera_preview_task_);
            camera_preview_task_ = nullptr;
            // Hide overlay by sending null image
            auto* muse = static_cast<MuseDisplay*>(display_);
            muse->SetPreviewImage(nullptr);
...
private:
    ...
    void StartNetwork() override {
        ...
        InitializeMuseMcpTools(chord_player_, static_cast<MuseDisplay*>(display_));
        ...
        try {
            ...
            switch (camera_type_) {
                case OTTO_CAMERA_OV3660:
                    ...
                    ESP_LOGI(TAG, "OV3660: VFlip=true, HMirror=true");
                    break;
                case OTTO_CAMERA_OV2640:
                default:
                    camera_->SetVFlip(true);
                    camera_->SetHMirror(false);
                    ESP_LOGI(TAG, "OV2640: VFlip=true, HMirror=false");
                    break;
            }
            return true;
        } catch (...) {
...
public:
    OttoRobot() : boot_button_(BOOT_BUTTON_GPIO),
...
#else
        #error "OTTO_HARDWARE_VERSION 设置无效，请使用 OTTO_VERSION_AUTO, OTTO_VERSION_CAMERA 或 OTTO_VERSION_NO_CAMERA"
#endif
        ...
        InitializeOttoController();   // 重新启用：提供手势 + 情绪动作支持
...
void OttoStartCameraPreview() {
    if (g_otto_robot) g_otto_robot->StartCameraPreview();
}
void OttoStopCameraPreview() {
    if (g_otto_robot) g_otto_robot->StopCameraPreview();
}
