...
void Protocol::SendUserText(const std::string& text) {
    // Step 1: open listening session in auto-stop mode
    std::string start_json = "{\"session_id\":\"" + session_id_ +
                             "\",\"type\":\"listen\",\"state\":\"start\",\"mode\":\"auto\"}";
    SendText(start_json);
    // Step 2: inject the user's text as recognized speech
    std::string detect_json = "{\"session_id\":\"" + session_id_ +
                              "\",\"type\":\"listen\",\"state\":\"detect\",\"text\":\"" + text + "\"}";
    SendText(detect_json);
}
